...
describe("Logs Routes Integration", () => {
  beforeEach(() => {
    ...
    (getUserRoleForFeed as jest.Mock).mockResolvedValue("admin");
  ...
  describe("GET /logs", () => {
    ...
    it("should apply role-based filtering for mods", async () => {
      ...
      (getUserRoleForFeed as jest.Mock).mockResolvedValue("mod");
      // Make the request
      const response = await request(app)
        .get("/api/logs")
        .query({ uri: "feed:1" })
    ...
    it("should reject regular users", async () => {
      ...
      (getUserRoleForFeed as jest.Mock).mockResolvedValue("user");
